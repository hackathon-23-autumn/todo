...
const MyTodoList: React.FC<Props> = ({ todos, deleteTodo, changeStatus }) => {
  return (
    ...
    {todos.map((todo, index) => (
      ...
      <li 
        className={`flex justify-between p-2 border-l-4 rounded shadow ${todo.completed ? 'bg-gray-400 border-blue-500' : 'bg-white border-blue-500'}`} 
        key={index}>
        <span>{todo.todo}</span>
